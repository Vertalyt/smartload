...
<template>
      ...
      >
        <div 
        v-for="b in access" :key="b[nameParam]"
...
const itemAccessComputed = computed( () => props.item_access )


const access = ref(itemAccessComputed.value)
...
const isCheck = (val) => {
  access.value = itemAccessComputed.value.map(l => {
    return {
      ...l,
      check: val
    }
