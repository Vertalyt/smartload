...
<template>
      ...
      >
        <div 
        v-for="b in itemAccessComputed" :key="b[nameParam]"
        class="flex my-2 mx-3 items-center min-h-[2rem] pl-[2rem] pr-2 text-sm sm:text-base sm:pr-[2rem] border border-gray-300 rounded-md">
            <input
              ...
              v-model="b.check"
              :checked="b.check"
              :disabled="b[nameParam] === 'id'"
              :class="{ 'opacity-20 border-none' : b[nameParam] === 'id' }"
            />
            <label
...
const itemAccessComputed = ref( props.item_access )


const selectedDatabases = () => {
  const editAccessBD = itemAccessComputed.value.filter(database => database.check).map(database => database[props.nameParam]);
  emit('edit', editAccessBD)
};


const isCheck = (val) => {
  itemAccessComputed.value = itemAccessComputed.value.map(l => {
    return {
      ...l,
      check: val
    }
